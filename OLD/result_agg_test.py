...
def main(args):
    # 使いたいデータセット一覧
    entities = [
            "005_UCR_Anomaly_DISTORTEDCIMIS44AirTemperature1", "113_UCR_Anomaly_CIMIS44AirTemperature1"
    ]
    #data_name = "synthetic/UCR_Anomaly/gpt-4o"
    data_name = "UCR_Anomaly_batch"
    ...
    for entity in entities:
        ...
        for (model, variant), row in double_df.iterrows():
            # 保存先ディレクトリ: results/json/variant/
            variant_dir = os.path.join("results_test", variant, entity)
            ...
            metric_dict = {
                ...
                "recall": row["recall"],
                "f1": row["f1"],
                "affi_precision": row["affi precision"],
                "affi_recall": row["affi recall"],
                "affi_f1": row["affi f1"],
            }
